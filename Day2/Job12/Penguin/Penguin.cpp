...
void Penguin::goToMeetupLocation(const std::string &location) {
    ...
    if (found != meetupLocationsToTreasures.end()) {
        ...
        if (rand() % 2 == 0) {
            std::cout << "Found treasure: " << found->second << " at location: " << location << "\n";
            // Remove the treasure from the location
            meetupLocationsToTreasures.erase(found);
        } else {
            std::cout << "No treasure found at location: " << location << "\n";
        }
    } else {
...
void Penguin::addTreasureLocation(const std::string &location) {
    treasureLocations.insert(location);

    // Add the location to the meetupLocationsToTreasures map
    meetupLocationsToTreasures[location] = "Treasure" + std::to_string(rand() % 100);
}

void Penguin::removeTreasureLocation(const std::string &location) {
...
int main() {
    ...
    penguin1->addTreasureLocation("Location1");
    penguin1->goToMeetupLocation("Location1");
    //
